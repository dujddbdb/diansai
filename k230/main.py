...
CROP_CENTER_Y = CAM_H // 2 - 77  # 裁剪区域中心Y坐标(全帧坐标系)，调大则裁剪区域下移，调小则上移
...
DISPLAY_EVERY_N = 3  # 每N帧刷新一次显示，调大则显示帧率低但节省CPU，调小则反之

# cv_lite矩形检测器参数
CV_LOW = 20  # Canny边缘检测低阈值，调小则检测到更多边缘但噪声增多，调大则反之
CV_HIGH = 55  # Canny边缘检测高阈值，调小则边缘连接更宽松，调大则更严格
...
CV_MIN_EDGE = 6  # 最小边长(像素)

# find_rects回退方案
FIND_RECTS_THRESHOLD = 4500  # 矩形检测阈值，调大则检测更少但更精准，调小则反之

# A4黑框靶标尺寸约束 (320x240检测裁剪)
OUTER_MIN_AREA = 900  # 外框最小面积(像素²)
OUTER_MAX_AREA = 70000  # 外框最大面积(像素²)
RECT_MIN_ANGLE = 48  # 矩形最小角度(度)，调大则对矩形角度要求更严格
ASPECT_MIN = 0.55  # 最小宽高比
ASPECT_MAX = 1.75  # 最大宽高比
BLACK_GRAY_TH = 95  # 黑色灰度阈值，调大则更多像素被判定为黑色，调小则反之
WHITE_GRAY_TH = 135  # 白色灰度阈值，调大则判定白色更严格，调小则更宽松
...
TRACK_FAST_FULL_RESCAN_PERIOD = 8  # 快速移动时全帧重扫周期(帧)
...
FULL_RESCAN_PERIOD = 18  # 全帧重扫周期(帧)，调小则更频繁全扫但耗时多
LOCK_HOLD_FRAMES = 14  # 锁定保持帧数，目标丢失后保持追踪的帧数
...
class FastRectangleTracker:
    ...
    def _scan_roi(self, gray, roi):
        ...
        if tracking_roi:
            ...
            if cands:
                return cands
        # 标准检测流程: cv_lite > blob > find_rects
