...
//  基础巡线参数
// ============================================================
#define BASE_RPM             60.0f   // 基础巡线转速(RPM)，直道巡航基准速度
                                    // 调大：整体速度更快，但过弯压力更大
                                    // 调小：整体速度更慢，更稳定
...
#define RAMP_STARTUP         0.1f    // 启动加速度斜率(RPM/ms)，启动时转速上升速率
                                    // 调大：启动更快更猛，但可能打滑
                                    // 调小：启动更平缓更稳，但加速慢
#define TRACK_STOP_RAMP_RPM_PER_MS  0.3f  // 达到目标圈数后的减速斜率(RPM/ms)
                                    // 达标后不再硬急停，先斜坡减速到0再断电
                                    // 调大：停车更快，但冲击更明显
                                    // 调小：停车更平稳，但滑行距离更长
...
//  巡线位置PID参数
// ============================================================
#define KP_NORMAL            0.065f  // 巡线PID比例系数，响应偏差的主要力度
                                    // 调大：响应更快，纠偏更有力，但可能超调振荡
                                    // 调小：响应更慢，更平稳，但纠偏不及时
#define KI_NORMAL            0.0f    // 巡线PID积分系数，消除稳态误差
                                    // 调大：消除静态偏差更快，但易引起超调和震荡
                                    // 调小：消除静态偏差更慢，系统更稳定
#define KD_NORMAL            0.50f   // 巡线PID微分系数，抑制超调和振荡
...
#define RIGHT_ANGLE_WHITE_MIN               8U    // 判定全白所需最少白通道数
                                                  ...
                                                  // 调大：全白判定更严格，不易误判
                                                  // 调小：全白判定更宽松，易触发
#define RIGHT_ANGLE_ABANDON_CONFIRM_SAMPLES 3U    // 放弃预触发确认采样数(去抖)
                                                  // 预触发期间"中间黑+边缘白"(已回到正常巡线)
                                                  // 连续多少个采样周期才确认放弃、退回空闲检测
                                                  // 调大：抗干扰强，但误触发占用时间更长
                                                  // 调小：放弃更快，但正常巡线抖动可能误放弃真直角

// ============================================================
//  直角转弯控制参数
// ============================================================
#define CORNER_YAW_TARGET             90.0f   // 转弯目标偏航角度(度)，90度直角弯
#define CORNER_IMU_ENTRY_RAMP_DEG     18.0f   // 直角入口IMU力道平滑爬升角度
#define CORNER_YAW_EXIT_EPSILON_DEG   3.0f    // 出弯判据：剩余偏航误差小于此值即退出直角状态机
                                              // 直角全程由IMU执行，无灰度接管，退出即是完成
                                              // 调大：更早退出，但转弯精度略降
                                              // 调小：收敛要求更严，转弯更精确但可能因噪声难以收敛
#define CORNER_IMU_FORCE_MIN_SCALE    0.20f   // IMU二次力道包络两端保底比例
                                              // 调大：起步/收尾更有力，但更容易突兀
                                              // 调小：起步/收尾更柔，但太小可能转不起来
...
#define KD_CORNER_YAW                 22.0f   // 转弯偏航PID微分系数
                                              // 调大：抑制超调更好，更稳，但对噪声敏感
                                              // 调小：响应更快，但易超调振荡
#define CORNER_TURN_RPM               60.0f   // 转弯基础转速(RPM)，转弯时的基准速度
                                              // 调大：转弯更快，但离心力大易冲出
                                              // 调小：转弯更慢，更稳定，但耗时久
#define CORNER_MIN_RPM                20.0f   // 转弯最低转速(RPM)，差速时内侧轮最低速度
                                              // 调大：内侧轮速度不太低，出弯加速快
                                              // 调小：内侧轮更慢，转弯半径更小
#define CORNER_MAX_RPM                80.0f   // 转弯最高转速(RPM)，差速时外侧轮最高速度
                                              // 调大：外侧轮更快，转弯更猛
                                              // 调小：外侧轮更慢，更平稳
