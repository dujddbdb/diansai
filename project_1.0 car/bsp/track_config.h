...
// 基础巡线参数
#define BASE_RPM             80.0f   // 基础巡线转速(RPM)，越大直道越快
#define STRAIGHT_RPM_LIMIT   150.0f  // 直道最高转速限制(RPM)，上限保护
#define RAMP_STARTUP         0.1f    // 启动加速度斜率(RPM/ms)，越大启动越快

// 巡线位置PID参数
#define KP_NORMAL            0.055f  // 巡线PID比例系数，越大响应越快但易超调
#define KI_NORMAL            0.0f    // 巡线PID积分系数，越大稳态误差越小但易振荡
#define KD_NORMAL            0.20f   // 巡线PID微分系数，越大抑制超调越强但易抖动
#define ERR_FILTER_ALPHA     0.9f    // 巡线误差低通滤波系数(0-1，越大滤波越弱)
#define PID_FILTER_ALPHA     0.80f   // 巡线PID输出低通滤波系数(0-1，越大滤波越弱)

// 灰度传感器配置
#define GRAY_DIRECTION       1       // 0-通道0对应传感器1(左到右) 1-反向
#define GRAY_THRESHOLD_SHIFT 0.6f    // 灰度二值化阈值偏移量(0-1，越大越容易判黑)
#define GRAY_ANALOG_EMA_ENABLE        1     // 0-关闭模拟量EMA滤波 1-开启
#define GRAY_ANALOG_EMA_PREV_WEIGHT   7     // EMA旧值权重，越大越平滑但响应越慢
#define GRAY_ANALOG_EMA_NEW_WEIGHT    3     // EMA新值权重，越大响应越快但噪声越大
#define GRAY_ANALOG_EMA_TOTAL_WEIGHT  10    // EMA权重总和(PREV+NEW)

// 直角检测参数
#define RIGHT_ANGLE_BLACK_CONFIRM_SAMPLES   8U    // 黑电平确认采样数(去抖)，越大越稳定但越慢
#define RIGHT_ANGLE_FEATURE_CONFIRM_SAMPLES 1U    // 直角特征确认采样数
#define RIGHT_ANGLE_WHITE_CONFIRM_SAMPLES   2U    // 全白确认采样数，越大越稳定但越慢
...
#define KP_CORNER_YAW                 1.85f   // 转弯偏航PID比例系数，越大转得越猛
#define KD_CORNER_YAW                 22.0f   // 转弯偏航PID微分系数，越大抑制超调越强
#define CORNER_TURN_RPM               40.0f   // 转弯基础转速(RPM)，越大弯中越快
#define CORNER_MIN_RPM                20.0f   // 转弯最低转速(RPM)，下限保护
#define CORNER_MAX_RPM                92.0f   // 转弯最高转速(RPM)，上限保护

// 转弯过渡曲线参数
#define CORNER_PID_SMOOTH_ALPHA           0.30f   // 转弯PID平滑滤波系数
#define CORNER_PREVIEW_DECEL_RPM          16.0f   // 预览减速转速量(RPM)，越大入弯前减速越多
#define CORNER_ENTRY_BLEND_PROGRESS       0.30f   // IMU差速在前30%角度进度内平滑拉起
#define CORNER_SPEED_EXIT_START_PROGRESS  0.55f   // 出弯加速起始进度(0-1)，越大越晚开始加速
#define CORNER_SPEED_EXIT_BLEND_WIDTH     0.45f   // 出弯加速过渡宽度
#define CORNER_GYRO_DIFF_LIMIT_RPM        65.0f   // 转弯陀螺仪差速限幅(RPM)
#define CORNER_DIFF_SLEW_UP_RPM_PER_MS    1.15f   // 转弯差速上升斜率(RPM/ms)，越大差速建立越快
#define CORNER_BASE_SLEW_DOWN_RPM_PER_MS  0.50f   // 转弯基础转速下降斜率(RPM/ms)，越大减速越快
#define CORNER_BASE_SLEW_UP_RPM_PER_MS    0.25f   // 转弯基础转速上升斜率(RPM/ms)，越大加速越快
#define WHEEL_COMMAND_SLEW_RPM_PER_MS     2.20f   // 轮指令斜率限制(RPM/ms)，越大响应越快

// 陀螺仪参数
#define GYRO_YAW_DIRECTION    1           // 陀螺仪偏航方向系数(±1)
#define GYRO_STRAIGHT_DAMPING_ENABLE  1   // 0-关闭直道陀螺仪阻尼 1-开启
#define KD_GYRO_STRAIGHT              5.5f   // 直道陀螺仪阻尼微分系数，越大阻尼越强
#define GYRO_STRAIGHT_LIMIT           15.0f  // 直道陀螺仪阻尼输出限幅(RPM)
#define GYRO_MAIN_POLL_PERIOD_MS      1U    // 陀螺仪主轮询周期(ms)，越小更新越快
